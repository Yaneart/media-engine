...
Original-torrent operations emit JSON structured logs with an `original_torrent` component and a `runtime`, `session`, or `stream` scope. The allowlisted fields cover operation/metadata duration, upstream header wait, first body byte, full/partial Range offsets, success/failure/cancellation, active session/creation/stream counts, resource/reference counts, ownership class, file count, and cleanup. Capability/session IDs, info hashes, magnets, torrent payloads, titles and file paths, internal URLs, credentials, and raw error messages are never logged. Telemetry sink failures are isolated from control and streaming behavior.
