...
Браузер использует `@media-engine/sdk` для public discovery и безопасных metadata torrent-провайдеров. Релизы группируются в русскоязычные, международные и fallback-каталоги, затем по настоящему display name провайдера. Для кандидатов JacRed также показывается конкретный upstream-каталог, например BitRu или RuTracker, если JacRed его сообщил. Эти подписи каталога не гарантируют фактический язык аудиодорожки релиза. Вызовы create/status/select/stop torrent-session проходят через same-origin server-side BFF, который добавляет `MEDIA_ENGINE_ORIGINAL_TORRENT_TOKEN`; token не раскрывается через переменную с префиксом `VITE_` и не попадает в browser bundle. Вне Compose example server обращается к API по `MEDIA_ENGINE_ORIGINAL_TORRENT_API_URL`, по умолчанию `http://127.0.0.1:3000`.
