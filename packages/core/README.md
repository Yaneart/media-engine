...
Title search distinguishes primary discovery from slower fallback identity sources through optional `capabilities.search.titleDiscovery`. Custom providers are primary by default. Fallback providers run only after primary typo broadening and only when the remaining result is empty or has conflicting exact-title identities. External-ID search still calls every compatible provider immediately.
