...
Core также экспортирует контракт canonical identity. `CanonicalIdentityIndex` назначает
непрозрачный provider-independent `workKey`, сохраняет его при добавлении проверенных внешних ID,
разрешает старые aliases вида `imdb:tt0816692` и отклоняет конфликтующие mappings. Встроенный индекс
работает внутри процесса; приложениям с долговечной identity нужно сохранять эту модель ключей и
aliases в своём registry.
...
Если streaming-провайдеру нужен отсутствующий в availability-запросе внешний ID, Core может через
подключённые metadata-провайдеры найти один однозначно подтверждённый ID до выбора источников.
Передавайте каноническое название, тип медиа и желательно точный год вместе с уже известными ID.
При конфликтующей идентичности движок ничего не угадывает.
