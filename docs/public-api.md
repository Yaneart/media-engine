...
With engine debug mode enabled, built-in search results include an optional `ranking` object. It exposes the selected ranking formula, match strength, strongest title-match category/value, and each normalized signal as `{ value, weight, contribution }`, plus the pre-bounded total. `scorePosition`, `diversityPosition`, and `finalPosition` distinguish raw score order, the bounded diversity pass, and later identity-snapshot recovery. `diversity.adjusted` and its normalized family explain a diversity move. Normal responses omit this object, and custom merge strategies are not required to provide it.
