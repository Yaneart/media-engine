...
8. Audit live source filtering breadth.
   ...
   - `pnpm smoke:source-filter-audit` now covers 10 movies, 10 series, and 10 anime with provider-level filter reasons plus expected Kinopoisk ID and title assertions; `--category` runs one media category independently.
   ...
   - The 10-anime strict audit passed after correcting fixture IDs and Japanese title aliases. Collaps, Kodik, and Vibix remained available across all cases, while FlixCDN was preserved in 9/10. ASHDI was accepted when live validation succeeded and filtered when it failed, avoiding a permanent provider-wide block. No mistakenly filtered working source was found in the anime sample.
